...
;; use eglot to navigate in code.
;; M-. (Alt + Period): Jump to Definition.
;; M-, (Alt + Comma): Pop / Go Back
;; M-? (Alt + Question Mark): Find all References.
;;
;(add-hook 'c-mode-hook 'eglot-ensure)
; (add-hook 'c++-mode-hook 'eglot-ensure)
; (add-hook 'python-mode-hook 'eglot-ensure)
;; 1. Load the built-in Emacs cscope engine

(use-package xcscope
  :ensure t
  :init
  ;; Silence the Emacs 30 legacy byte-compiler warning completely
  (with-no-warnings
    (defun cscope-event-window (event) nil))
  (cscope-setup)

  :hook
  ;; 3. Automatically activate cscope whenever you open a C or C++ file
  (c-mode-common . cscope-minor-mode))
